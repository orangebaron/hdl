    ...
    for (auto g: gates) {
      ...
      { char counter = 0;
        for (auto p: otps) {
          ...
          try { values.at(g.otpNames[counter]); } catch (...) { alreadyAssigned = false; }
          if (alreadyAssigned) throw std::invalid_argument(getName()+": pin assigned to twice");
          values[g.otpNames[counter++]] = p;
        }
      }
    }
    PinValues otpValues;
